...
\caption{Caracteristicas morfologicas de una planta con flor.}
\label{planta}
\end{figure}
...
Calcule la matriz de covarianza $\Sigma$ de las variables medidas en esta investigación e interpretela.
...
\begin{enumerate}
\item Calcule la matriz de covarianza $\Sigma (X_{1},X_{2},X_{3})$ e interpretela.
\item Diagonalize la matriz obtenida y determine la matriz de rotación $R$ que diagonaliza a $\Sigma$
\item Usando la matriz $R$, determine las nuevas variables $\tilde{X}_{1},\tilde{X}_{2}$ y $\tilde{X}_{3}$.
\end{enumerate}

\item ESTIMACION DE MEDIDA FALTANTE
